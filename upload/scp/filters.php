...
if($filter || ($_REQUEST['a'] && !strcasecmp($_REQUEST['a'],'add'))) {
  ...
  if($dept && !$dept->isActive())
    $warn = sprintf(__('%s is assigned a %s that is not active.'), __('Request Filter'), __('Department'));

  if($topic && !$topic->isActive())
    $warn = sprintf(__('%s is assigned a %s that is not active.'), __('Request Filter'), __('Help Topic'));

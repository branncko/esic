...
if ($settings['status'])
    ...
    case 'open':
    case 'closed':
		$results_type = ($status == 'closed') ? __('Closed Requests') : __('Open Requests');
...
<div id="tickets-new">
	...
	<form action="tickets.php" method="get" id="ticketSearchForm">
		...
		<input class="btn" type="submit" value="<?php echo __('Search');?>">
	<div class="pull-right">
		<?php echo __('By Type'); ?>:
		<select name="topic_id" class="nowarn" onchange="javascript: this.form.submit(); ">
			<option value="">&mdash; <?php echo __('All Request Types');?> &mdash;</option>
	<?php
	...
	<table id="ticketTable" width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<thead>
			<tr>
				<th nowrap width="20%">
					<a href="tickets.php?sort=ID&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Ticket ID"><?php echo __('Request #');?>&nbsp;<i class="icon-sort"></i></a>
				</th>
				<th width="20%">
		...
		<?php
		 ...
		 if ($tickets->exists(true)) {
			 foreach ($tickets as $T) {
				...
				?>
				<tr id="<?php echo $T['ticket_id']; ?>" class="tr-height">
					<td>

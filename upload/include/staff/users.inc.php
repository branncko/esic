 ...
 <table class="list" border="0" cellspacing="1" cellpadding="0">
    <thead>
        <tr>
            ...
            <th width="30%"><a  <?php echo $status_sort; ?> href="users.php?<?php
                echo $qstr; ?>&sort=status"><?php echo __('Status'); ?></a></th>
            <th width="16%"><a <?php echo $create_sort; ?> href="users.php?<?php
                echo $qstr; ?>&sort=create"><?php echo __('Created'); ?></a></th>
            <th width="16%"><a <?php echo $update_sort; ?> href="users.php?<?php
        ...
        foreach ($users as $U) {
               ...
               <tr id="<?php echo $U['id']; ?>">
                ...
                <td>&nbsp;
                    ...
                    <?php
                    if ($U['ticket_count'])
                         echo sprintf('<span class="numberst">%d', $U['ticket_count']);
        ...
        if (ids.length) {
          ...
          if (action === 'delete') {
              options['deletetickets']
                =  __('Serão deletadas todas as Solicitações e Arquivos deste Solicitante!');
          }
          if (!confirmed)
              $.confirm(__('Você tem certeza?'), undefined, options).then(submit);
          else
              submit();
        }
        else {

...
class StaffNav {
    ...
    function getTabs(){
        ...
        if(!$this->tabs) {
            ...
            $this->tabs['tickets'] = array('desc'=>__('Requests'),'href'=>'tickets.php','title'=>__('Ticket Queue'));
        ...
        foreach($this->getTabs() as $k=>$tab){
            ...
            switch(strtolower($k)){
                ...
                case 'settings':
                    ...
                    $subnav[]=array('desc'=>__('Requests'),'href'=>'settings.php?t=tickets','iconclass'=>'ticket-settings');
                ...
                case 'manage':
                    ...
                    $subnav[]=array('desc'=>__('Request Filters'),'href'=>'filters.php',
                                        'title'=>__('Request Filters'),'iconclass'=>'ticketFilters');
    ...
    function getNavLinks(){
        ...
        if(!$this->navs){
            ...
            if ($cfg->getClientRegistrationMode() != 'disabled'
                ...
                $navs['new']=array('desc'=>__('Open a New Request'),'href'=>'open.php','title'=>'');
            if($user && $user->isValid()) {
                if(!$user->isGuest()) {
                    $navs['tickets']=array('desc'=>sprintf(__('Requests (%d)'),$user->getNumTickets($user->canSeeOrgTickets())),
                                           'href'=>'tickets.php',
                                            'title'=>__('Show all requests'));
                } else {
                    $navs['tickets']=array('desc'=>__('View Request Thread'),
                                           'href'=>sprintf('tickets.php?id=%d',$user->getTicketId()),
                                           'title'=>__('View request status'));
                }
            } else {
                $navs['status']=array('desc'=>__('Check Request Status'),'href'=>'view.php','title'=>'');
            }
            $this->navs=$navs;
        }
